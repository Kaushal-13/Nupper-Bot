...
def nup(input_pdf_path, output_pdf_path, is_landscape=False, rows=2, cols=2):
    ...
    for i in range(0, num_pages, pages_per_sheet):
        ...
        positions = [(col * page_width, (rows - 1 - row) * page_height)
        ...
        for j in range(pages_per_sheet):
            if i + j < num_pages:
                ...
                translation_matrix = [
                    1, 0, 0, 1, position[0] + x_offset, position[1] + y_offset]
